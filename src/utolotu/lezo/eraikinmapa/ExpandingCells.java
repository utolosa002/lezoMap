...
public class ExpandingCells extends Activity {
  ...
  Intent in = getIntent();
  String izen = in.getStringExtra("titulua");
  String desk = in.getStringExtra("Snippet");
		// getting attached intent data
//    	ExpandableListItem[] values = new ExpandableListItem[] {
//                new ExpandableListItem("Gezala", R.drawable.arts, CELL_DEFAULT_HEIGHT,
...
//        };
 		...
 		mData.add(new ExpandableListItem(izen,  MainActivity.guneLista.get(1).image,
				CELL_DEFAULT_HEIGHT,desk));
		for (int i = 0; i<MainActivity.guneLista.size();i++){
			if (MainActivity.guneLista.get(i).name!=MainActivity.guneLista.get(1).name){
			mData.add(new ExpandableListItem(MainActivity.guneLista.get(i).name, MainActivity.guneLista.get(i).image,
					CELL_DEFAULT_HEIGHT,MainActivity.guneLista.get(i).description));
			}
    	}
//        List<ExpandableListItem> mData = new ArrayList<ExpandableListItem>();
//
